...
import { autoRearrange } from './layout.js';
...
export function setupEventListeners(instance, zoomPanControls) {
    ...
    [addVectorBtn, addScalarBtn, addMatrixBtn].forEach(btn => {
    ...
    const handleDragStart = (e, type, op = null) => {
        e.dataTransfer.setData('node-type', type);
        if (op) e.dataTransfer.setData('node-op', op);
        e.dataTransfer.effectAllowed = 'copy';
    };

    addVectorBtn.addEventListener('dragstart', (e) => handleDragStart(e, 'vector'));
    addScalarBtn.addEventListener('dragstart', (e) => handleDragStart(e, 'scalar'));
    addMatrixBtn.addEventListener('dragstart', (e) => handleDragStart(e, 'matrix'));

    if (rearrangeBtn) {
        rearrangeBtn.addEventListener('click', () => {
            autoRearrange(instance);
        });
    }
    ...
    operationButtons.forEach(button => {
        button.addEventListener('dragstart', (e) => {
            ...
            handleDragStart(e, 'operation', op);
        });
    });

    // Workspace drag and drop listeners
    ...
    workspace.addEventListener('drop', (e) => {
        ...
        const op = e.dataTransfer.getData('node-op');
        ...
        const scale = zoomPanControls.getScale();
        const tx = zoomPanControls.getTranslateX();
        const ty = zoomPanControls.getTranslateY();

        const x = (mouseX - tx) / scale - 75; // Offset by half width (approx)
        const y = (mouseY - ty) / scale - 25; // Offset by half height (approx)

        const position = { x, y };

        if (type === 'vector') {
            addVectorNode(instance, false, position);
        } else if (type === 'scalar') {
            addScalarNode(instance, false, position);
        } else if (type === 'matrix') {
            addMatrixNode(instance, false, position);
        } else if (type === 'operation') {
            addOperationNode(instance, op, currentNodeType, position);
        }
