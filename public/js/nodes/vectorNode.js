...
import { checkMagneticConnection } from '../plumbSetup.js';

export function addVectorNode(instance, isResult = false, position = null) {
    ...
    const nodeElement = createNodeElement(id, 'vector', content, initialDisplayValue, isResult, position);

    instance.draggable(nodeElement, { 
        containment: 'parent',
        stop: (params) => checkMagneticConnection(id, instance)
    });

    instance.addEndpoint(id, { uuid: `${id}-in`, anchor: "Left", isTarget: true, maxConnections: 1, cssClass: "target-endpoint" });
    instance.addEndpoint(id, { uuid: `${id}-out`, anchor: "Right", isSource: true, cssClass: "source-endpoint" });
